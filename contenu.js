const PORTFOLIO_CONTENU = {

  // 📁 Dossier : projets-applications/
  application: [
    "photo_de_couverture_developpement.png",
  ],

  // 📁 Dossier : projets-creation/
  projets: [
    "photo_de_couverture_projets.png",
    "Projet_Smart_Parking.html",
    "projet-bpgo.html",
    "projet-fondation.html",
    "sante-connectee.html",
    "gestion-clientele.html",
    "gestion-salon-de-coiffure.html",
    "projet-vba.html",
  ],

  // 📁 Dossier : projets-excel/
  excel: [
    "photo_de_couverture_excel.png",
    "Base_Clients.png",
    "Bouton-création-stat-et-graphiques.png",
    "Bouton-envoi-mail.png",
    "Bouton-préparation-des-stats.png",
    "CENTRE-DE-PILOTAGE-DE-PROJET-V2.0.png",
    "Dashboard1.png",
    "Facture.png",
    "PLAN-DE-PROJET-AGILE.png",
    "Programme_Journalier.png",
    "SUIVI-DE-CHANTIER-ÉLECTRICITÉ.png",
    "Suivi_Taches2.png",
    "Tableau-aide-à-la-décision.png",
    "bouton-importation-data.png",
  ],

  // 📁 Dossier : projets-logos/
  logos: [
    "photo_de_couverture_logo.png",
    "Logo-marque-sans-gluten.png",
    "Logo-santé-conectée.png",
    "logo-patisserie.jpg",
    "logo_salle_de_sport.jpg",
  ],

  // 📁 Dossier : projets-affiches/
  affiches: [
    "photo_de_couverture_affiches.png",
    "Affiche-evenement-fondation.jpeg",
    "Affiche-evenement-fondation2.jpeg",
    "Affiche-evenement-master2.png",
    "https://drive.google.com/file/d/1VDnWFzGi1ABleP1oqHCD9xTJhoWBLNtj/preview",
  ],

  // 📁 Dossier : projets-cv/
  cv: [
    "photo_de_couverture_cv.png",
  ],

  // 📁 Dossier : projets-publications/
  publications: [
    "photo_de_couverture_publications.png",
    "Publication-gym.png",
    "Publication-gym2.png",
    "Publication-salle-de-sport.png",
    "Publication-salle-desport3.png",
    "logo-couverture-patisserie.png",
    "logo-couverture-salle-sport.png",
    "post-instagram-1.png",
    "we_are_open.png",
  ],

  // 📁 Dossier : projets-actualites/
  actualites: [
    "application.png",
    "secret_fashion_actualité_1.png",
    "secret_fashion_actualité_2.png",
    "secret_fashion_actualité_3.png",
    "secret_fashion_actualité_4.png",
    "Vendly_actualité",
  ],

};
